...
class Mapbox extends React.Component {
  ...
  componentDidMount() {
    ...
    mapboxgl.accessToken = 'pk.eyJ1IjoidmlzdXJpaGV3YWdlIiwiYSI6ImNsdW04dmRzODEyamcycWxpamw4eW80b3gifQ.OdN2iZRRxRGkiBHSrZbOjQ'; // Replace with your Mapbox access token
  ...
  this.map.fitBounds(sriLankaBounds, {
    ...
    this.updateWeatherInterval = setInterval(this.updateWeatherData, 30000); //5 min intervals before generating
  }
  ...
  updateWeatherData = async () => {
    try {
      ...
      const response = await axios.get('https://weatherapprepo-74z5.onrender.com/weather', {
          headers: {
      ...
      await axios.put(`https://weatherapprepo-74z5.onrender.com/weather/${randomDistrict.latitude}/${randomDistrict.longitude}`, {
  ...
  handleMapClick = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://weatherapprepo-74z5.onrender.com/weather/${lngLat.lat}/${lngLat.lng}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
